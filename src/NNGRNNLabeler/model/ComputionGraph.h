...
public:
	inline void initial(ModelParams& model, HyperParams& opts, AlignedMemoryPool* mem = NULL){
		...
		_grnn.init(&model.grnn_project, opts.dropProb, true, mem);
...
public:
	...
	inline void forward(const Feature& feature, bool bTrain = false){
		...
		_grnn.forward(this, getPNodes(_word_represents, words_num));
		_max_pooling.forward(this, getPNodes(_grnn._output, words_num));
		_min_pooling.forward(this, getPNodes(_grnn._output, words_num));
		_avg_pooling.forward(this, getPNodes(_grnn._output, words_num));
